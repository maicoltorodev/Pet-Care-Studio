...
export function BeforeAfterDesktop({ content, transformations }: { content: ContentItem[], transformations: Transformation[] }) {
    const header = {
    ...
    return (
        <section id="resultados" className="cinematic-section border-b border-white/5 py-32">
                ...
                {visibleTransformations.length > 0 ? (
                    <div className="grid gap-10 lg:grid-cols-3">
                        {visibleTransformations.map((item: Transformation, index: number) => (
                            <div
                                key={item.id}
                                className="group cinematic-card hover:-translate-y-2 transition-transform duration-700"
